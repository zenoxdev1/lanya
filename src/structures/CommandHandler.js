...
class CommandHandler {
  ...
  async handleMessage(message) {
    ...
    if (!rawContent) {
      return message.channel.send(
        "👋 Hi there! Run `@Miyako help` to see all I can do or browse the slash commands by typing `/`"
      );
    }
  ...
  async handleInteraction(interaction) {
    ...
    if (!command) {
      this.client.log.warn(
        `Command '${interaction.commandName}' not implemented.`
      );
  ...
  async runChecks(ctx, command) {
    ...
    if (command.guildOnly && !ctx.guild) {
      await ctx.reply({
        content:
          "Ba-baka! What do you think you're doing in my DMs? That command can only be used in a server!",
      });
  ...
  async checkPermissions(ctx, command) {
    ...
    if (missing.length) {
      await ctx.reply({
        content: `I need the following permission${plural(
          missing
        )} to run that command: **${missing.join(", ")}**`,
      });
    ...
    if (user.length) {
      await ctx.reply({
        content: `You need the following permission${plural(
          user
        )} to run that command: **${user.join(", ")}**`,
      });
