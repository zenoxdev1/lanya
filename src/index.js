...
axios
    .get('https://api.github.com/repos/birajrai/lanya/releases/latest')
    .then(res => {
...
console.log(`\u001b[0m`);
console.log(chalk.red(`© Lanya | 2021 - ${new Date().getFullYear()}`));
...
process.on('unhandledRejection', error => {
    ...
    const embed = new Discord.EmbedBuilder().setTitle(`🚨・Unhandled promise rejection`).addFields([
        {
            name: 'Error',
...
process.on('warning', warn => {
    ...
    const embed = new Discord.EmbedBuilder().setTitle(`🚨・New warning found`).addFields([
        {
            name: `Warn`,
            value: `\`\`\`${warn}\`\`\``,
        },
