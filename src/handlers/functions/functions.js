...
module.exports = async client => {
    ...
    client.checkPerms = async function ({ flags: flags, perms: perms }, interaction) {
    ...
    client.checkBotPerms = async function ({ flags: flags, perms: perms }, interaction) {
        for (let i = 0; i < flags.length; i++) {
            if (!interaction.guild.members.me.permissions.has(flags[i])) {
                client.errNoPerms(
                    {
    ...
    client.checkUserPerms = async function ({ flags: flags, perms: perms }, interaction) {
        for (let i = 0; i < flags.length; i++) {
            if (!interaction.member.permissions.has(flags[i])) {
                client.errMissingPerms(
                    {
    ...
    client.loadSubcommands = async function (client, interaction, args) {
        try {
            ...
            if (data.Beta == true) {
                ...
                }/${interaction.options.getSubcommand()}-beta`)(client, interaction, args).catch(err => {
                    client.emit('errorCreate', err, interaction.commandName, interaction);
                });
            } else {
                return require(`${process.cwd()}/src/commands/${
                    interaction.commandName
                }/${interaction.options.getSubcommand()}`)(client, interaction, args).catch(err => {
                    client.emit('errorCreate', err, interaction.commandName, interaction);
                });
            }
        } catch {
    ...
    client.generateEmbed = async function (start, end, lb, title, interaction) {
        ...
        let embed = client.templateEmbed().setTitle(`${title}`).setDescription(`${result.toString()}`);
    ...
    client.createLeaderboard = async function (title, lb, interaction) {
        interaction
            .editReply({ embeds: [await client.generateEmbed(0, 0, lb, title, interaction)], fetchReply: true })
            .then(async msg => {
                ...
                collector.on('collect', async btn => {
                    if (btn.user.id == interaction.user.id && btn.message.id == msg.id) {
                        ...
                        msg.edit({
                            embeds: [await client.generateEmbed(currentIndex, currentIndex, lb, title, interaction)],
                            components: [row2],
                        });
                        btn.deferUpdate();
                    }
                ...
                collector.on('end', async btn => {
                    ...
                    let rowDisable = new Discord.ActionRowBuilder().addComponents(btn1Disable, btn2Disable);
