...
class Wolfram extends Command {
  ...
  async run(ctx, options) {
    ...
    const query = this.aliases.includes(ctx.alias)
      ? `${toProperCase(ctx.alias)} ${options.getString("query")}`
    ...
    if (!pods || pods.error)
      return ctx.reply("Couldn't find an answer to that question!");
    const description = pods[1].subpods[0].plaintext.substring(0, 1980);

    const embed = this.client.embed().setTitle(pods[0].subpods[0].plaintext);
