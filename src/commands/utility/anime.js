...
class Anime extends Command {
  ...
  async run(ctx, options) {
    ...
    if (!res)
      return ctx.reply(
        `Invalid page ${page} there is only ${data.length} pages.`
      );
    ...
    const embed = this.client
      ...
      .setTitle(
        res.attributes.titles.en
          ? `${res.attributes.titles.en} (Japanese: ${res.attributes.titles.en_jp})`
          : res.attributes.titles.en_jp
      )
      .setDescription(res.attributes.synopsis)
      .addFields({
        name: "Age Rating",
        value: `${res.attributes.ageRating}${
      ...
      .setThumbnail(
        res.attributes.posterImage && res.attributes.posterImage.original
      )
