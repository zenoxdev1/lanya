...
class GitHub extends Command {
  ...
  async run(ctx, options) {
    ...
    if (!username || !repository)
      return ctx.reply("Repository must be in the form `username/repository`");

    const body = await request(
      `https://api.github.com/repos/${username}/${repository}`,
      {
        headers: { "User-Agent": "lanya https://github.com/birajrai/lanya" },
      }
    )
    ...
    if (!body)
      return ctx.reply("Could not fetch that repo, are you sure it exists?");
    ...
    if (body.fork)
      footer.push(
        `❯ **Forked** from [${body.parent.full_name}](${body.parent.html_url})`
      );
